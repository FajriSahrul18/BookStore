...
<head>
  ...
  <title>Penerbit - BookStore</title>
</head>
...
<body>
  ...
  <script>
    function hapus(id) {
      $.ajax({
        url: '../model/penerbit.php?action=delete',
        type: 'POST',
        data: {
          id: id
        },
    ...
    function getCategory(page) {
      ...
      $.ajax({
        url: '../model/penerbit.php?page=' + page,
        type: 'GET',
        success: function(data) {
          var no = page * 4 - 3;

          $.each(data.penerbit, function(i, item) {
            $('#table-penerbit').append(
              '<tr>' +
              '<td>' + (no + i) + '</td>' +
              '<td>' + item.nama + '</td>' +
              '<td>' + item.alamat + '</td>' +
